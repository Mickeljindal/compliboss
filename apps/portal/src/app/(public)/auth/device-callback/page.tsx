...
export default function DeviceCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!callbackPort || !state) {
      setStatus('error');
      setErrorMessage('Missing required parameters. Please try signing in again from the CompliBoss agent.');
      return;
    }
    ...
    if (Number.isNaN(port) || port < 1 || port > 65535) {
      setStatus('error');
      setErrorMessage('Invalid callback port. Please try signing in again from the CompliBoss agent.');
      return;
    }
      ...
      try {
        ...
        setErrorMessage(
          err instanceof Error
            ? err.message
            : 'Failed to complete sign-in. Please try again from the CompliBoss agent.',
        );
      }
    }
            ...
            {status === 'redirecting' && (
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">
                  Redirecting to the CompliBoss agent...
                </p>
              </div>
            )}
            {status === 'success' && (
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">
                  You can close this tab and return to the CompliBoss agent.
                </p>
              </div>
            )}
            {status === 'error' && (
