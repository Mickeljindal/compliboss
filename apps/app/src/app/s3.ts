...
// When STORAGE_DRIVER=local the object store is the API server's own disk, which
// this (separate) app process can't reach directly — file features route through
// the API instead. Point APP_AWS_ENDPOINT at a self-hosted S3-compatible store
// (e.g. MinIO) to enable the app's own S3 features without AWS. Either way, a
// missing S3 config here is expected and not a fatal error.
const STORAGE_DRIVER = process.env.STORAGE_DRIVER;

try {
  if (STORAGE_DRIVER === 'local') {
    // Local driver: no app-side S3 client. Not an error — expected in this mode.
    ...
    console.warn(
      '[S3] AWS/S3 not configured. Set APP_AWS_* (or APP_AWS_ENDPOINT for MinIO), or STORAGE_DRIVER=local. File features are disabled until then.',
    );
    s3ClientInstance = null as any;
  } else {
  ...
  console.warn('[S3] Failed to initialize S3 client; file features disabled.', error);
  // Dummy client — fail gracefully at call time instead of crashing at import.
  s3ClientInstance = null as any;
}
