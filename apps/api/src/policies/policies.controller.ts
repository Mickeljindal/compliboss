...
export class PoliciesController {
  ...
  async getPdfSignedUrl(
    ...
    // Generate signed URL (S3 presigned GET, or a signed local file URL).
    ...
    if (!isLocalStorage() && !process.env.APP_AWS_BUCKET_NAME) {
      return { url: null };
    }
    const url = await createDownloadUrl({
      bucket: resolveUploadsBucket(),
      key: pdfUrl,
  ...
  async uploadPolicyPdf(
    ...
    const bucketName = resolveUploadsBucket();
    ...
    if (oldPdfUrl && oldPdfUrl !== s3Key) {
      try {
        await deleteObject({ bucket: bucketName, key: oldPdfUrl });
      } catch {
        /* ignore */
      }
    }
  ...
  async deletePolicyPdf(
    ...
    if (version.pdfUrl) {
      try {
        await deleteObject({ bucket: bucketName, key: version.pdfUrl });
      } catch {
        /* ignore */
      }
      await db.$transaction([
  ...
  async getPdfUrl(
    ...
    if (!isLocalStorage() && !process.env.APP_AWS_BUCKET_NAME) return { url: null };
