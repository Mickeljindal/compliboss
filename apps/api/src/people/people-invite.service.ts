...
export class PeopleInviteService {
  ...
  private async addEmployeeWithoutInvite(
    ...
    if (sendPortalEmail) {
      try {
        ...
        await triggerEmail({
          to: email,
          subject: `You've been invited to join ${organization.name} on CompliBoss`,
  ...
  async resendPortalInvite(params: {
    ...
    await triggerEmail({
      to: email,
      subject: `Access your ${member.organization.name} Employee Portal on CompliBoss`,
      react: InvitePortalEmail({
  ...
  private async sendInviteEmails(params: {
    ...
    if (sendAppEmail) {
      await triggerEmail({
        to: email,
        subject: `You've been invited to join ${organizationName} on CompliBoss`,
        react: InviteEmail({
          organizationName,
          inviteLink: appLink,
