...
async function bootstrap(): Promise<void> {
  ...
  // Local filesystem upload endpoint (STORAGE_DRIVER=local) receives raw binary
  // bytes via a plain PUT — parse the whole body into a Buffer on req.body,
  // regardless of Content-Type, so the controller can write it straight to disk.
  const rawBinaryParser = express.raw({ type: () => true, limit: '150mb' });
  const isLocalUploadPath = (req: express.Request): boolean =>
    req.method === 'PUT' && req.path.endsWith('/uploads/local');
  ...
  app.use(
    (
      ...
      if (isLocalUploadPath(req)) {
        return rawBinaryParser(req, res, next);
      }
