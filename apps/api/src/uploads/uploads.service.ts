...
import { buildLocalUploadUrl } from '../app/storage/local-storage';
import {
  CreateUploadUrlDto,
...
export class UploadsService {
  ...
  async createUploadUrl(
    ...
    // Local filesystem driver: hand back a signed URL that points at our own
    // public upload endpoint (POST bytes there) instead of an S3 presigned URL.
    // The key layout and the returned shape are identical, so feature endpoints
    // that consume `s3Key` don't care which backend is active.
    if (isLocalStorage()) {
      const { uploadUrl } = buildLocalUploadUrl({
        bucket,
        key: s3Key,
        expiresInSeconds: expiresIn,
      });
      this.logger.log(
        `Issued local upload URL for org ${organizationId} (purpose=${dto.purpose})`,
      );
  ...
  async readUploadAsBase64(
    ...
    try {
      contentLength = await getObjectContentLength(bucket, s3Key);
    } catch (error) {
      this.logger.warn(
    ...
    try {
      const buffer = await getObjectAsBuffer(bucket, s3Key);
