...
import { isLocalStorage } from './storage/storage.config';
import { readLocalObject, statLocalObjectSize } from './storage/local-storage';
...
try {
  if (isLocalStorage()) {
    // Local filesystem driver: no S3 client is needed. File reads/writes go
    // through app/storage/local-storage.ts instead.
    logger.log(
      '[Storage] STORAGE_DRIVER=local — using filesystem storage; S3 client not initialized.',
    );
...
export async function getObjectContentLength(
  ...
  if (isLocalStorage()) {
    return statLocalObjectSize(bucket, key);
  }
