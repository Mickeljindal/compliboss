...
export class OrgChartService {
  ...
  constructor() {
    ...
    // Storage is usable if we're on the local driver, or S3 is fully configured.
    this.storageAvailable = isLocalStorage() || (!!s3Client && !!BUCKET_NAME);
  }
  ...
  private async deleteS3Object(s3Key: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to delete storage object ${s3Key}:`, error);
    }
  }
}
