...
export class AttachmentsService {
  ...
  constructor(private readonly uploadsService: UploadsService) {
    ...
    // when STORAGE_DRIVER=local. All object operations go through the shared
    // storage helpers, so no S3 client is required at construction time.
  ...
  async deletePolicyVersionPdf(s3Key: string): Promise<void> {
    try {
      await deleteObject({ bucket: this.bucketName, key: s3Key });
    } catch (error) {
  ...
  /**
   * Upload a buffer to storage with a specific key (no auto-generated path)
   */
  async uploadBuffer(
    s3Key: string,
    buffer: Buffer,
  ...
  async getObjectBuffer(s3Key: string): Promise<Buffer> {
    return readObjectBuffer({ bucket: this.bucketName, key: s3Key });
  }
