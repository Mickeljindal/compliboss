...
export class TrustCustomFrameworkBadgeService {
  ...
  async uploadBadge(
    ...
    const bucket = resolveOrgAssetsBucket();
    if (!isLocalStorage() && (!s3Client || !APP_AWS_ORG_ASSETS_BUCKET)) {
    ...
    const signedUrl = await createDownloadUrl({
      bucket,
      key,
      expiresInSeconds: BADGE_SIGNED_URL_TTL_SECONDS,
      disposition: 'inline',
      contentType: fileType,
    });
