 ...
 *
 * For self-hosted deployments (e.g. CompliBoss on your own domain), set
 * COOKIE_DOMAIN explicitly (e.g. ".compliboss.com") so cross-subdomain
 * cookies work across app./portal./api. subdomains. Falls back to the
 * legacy host-based detection when COOKIE_DOMAIN is not set.
 */
...
export function isStaticTrustedOrigin(origin: string): boolean {
  ...
  try {
    ...
    // Self-hosted: trust all subdomains of a configured root domain
    // (e.g. PUBLIC_ROOT_DOMAIN="compliboss.com" trusts *.compliboss.com).
    const rootDomain = process.env.PUBLIC_ROOT_DOMAIN?.trim();
    if (
      rootDomain &&
...
const gramMcpClient =
  ...
  process.env.GRAM_OAUTH_REDIRECT_URI
    ? {
        ...
        name: 'CompliBoss MCP (Gram)',
        type: 'web' as const,
        disabled: false,
        redirectUrls: [process.env.GRAM_OAUTH_REDIRECT_URI],
        metadata: null,
        // First-party client: Gram is CompliBoss's own hosted MCP, so the user's
...
const mcpLoginPage =
  ...
  `${process.env.NEXT_PUBLIC_APP_URL ?? 'https://app.compliboss.com'}/auth`;
...
export const auth = betterAuth({
  ...
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user, url }) => {
      ...
      await triggerEmail({
        to: user.email,
        subject: 'Verify your email for CompliBoss',
    ...
    organization({
      ...
      async sendInvitationEmail(data) {
        ...
        const appUrl =
          ...
          'https://app.compliboss.com';
        const inviteLink = `${appUrl}/invite/${data.invitation.id}`;
        await triggerEmail({
          to: data.email,
          subject: `You've been invited to join ${data.organization.name} on CompliBoss`,
          react: InviteEmail({
    ...
    magicLink({
      ...
      sendMagicLink: async ({ email, url }) => {
        ...
        if (isDev) {
          ...
          console.log('[Auth][DEV] Magic link URL (open to sign in):', url);
        }
        try {
          await triggerEmail({
            to: email,
            subject: 'Login to CompliBoss',
            react: MagicLinkEmail({ email, url }),
          });
        } catch (err) {
          // In dev, email is often unconfigured (no RESEND_API_KEY). Don't break
          // sign-in — the link is logged above. In prod, email must work.
          if (isDev) {
            console.warn('[Auth][DEV] Email send failed; use the magic link URL logged above.');
          } else {
            throw err;
          }
        }
    ...
    emailOTP({
      ...
      async sendVerificationOTP({ email, otp }) {
        ...
        if (isDev) {
          ...
          console.log('[Auth][DEV] OTP code (enter this to sign in):', otp);
        }
        try {
          await triggerEmail({
            to: email,
            subject: 'One-Time Password for CompliBoss',
            react: OTPVerificationEmail({ email, otp }),
          });
        } catch (err) {
          // In dev, email is often unconfigured (no RESEND_API_KEY). Don't break
          // sign-in — the OTP is logged above. In prod, email must work.
          if (isDev) {
            console.warn('[Auth][DEV] Email send failed; use the OTP code logged above.');
          } else {
            throw err;
          }
        }
