...
description: "Connect Google Cloud Platform to CompliBoss over OAuth so findings from Security Command Center flow into your compliance posture."
---

## About the GCP integration

CompliBoss connects to Google Cloud Platform over **OAuth 2.0** and reads findings from **Security Command Center (SCC)**. No service account JSON key is uploaded — you sign in with a Google account that has access to the organization and projects you want to monitor.

After you connect, CompliBoss auto-detects your organization, lets you pick projects, then runs a setup step that enables the required APIs and verifies IAM access. Findings are mapped to common frameworks (SOC 2, ISO 27001, CIS GCP Foundations, PCI DSS, HIPAA).

<Note>
  Security Command Center is where all GCP findings originate. If SCC is not enabled at the organization, CompliBoss cannot read findings regardless of connection method.
</Note>
...
- **Real permissions**: The OAuth scope only enables API calls — actual access is gated by the **IAM roles** assigned to the signed-in account. CompliBoss only makes read calls.
...
- **Scope**: CompliBoss scans the projects you select inside the organization the signed-in account belongs to.
...
3. Permission to enable APIs on at least one project in the organization (so CompliBoss's setup step can enable SCC, Cloud Resource Manager, and Service Usage APIs if they are not already on)
4. Admin access to your CompliBoss workspace
...
<Steps>
  <Step title="Start the connection">
    In CompliBoss, go to **Cloud Tests → GCP → Connect**. Click **Sign in with Google**.
  </Step>
  <Step title="Authorize with Google">
    Sign in with a Google account that meets the prerequisites above and approve the consent screen. CompliBoss stores the resulting refresh token (never the password) so it can run scheduled scans.
  </Step>
  <Step title="Select projects">
    CompliBoss auto-detects your organization and lists the projects that account can access. Pick the ones you want scanned — findings are scoped to those projects.
  </Step>
  <Step title="Let auto-setup run">
    CompliBoss's setup guide runs automatically and shows a checklist:
...
CompliBoss consumes findings from Security Command Center across services including:
...
- **Read-only in practice** — CompliBoss only issues read API calls against SCC and resource manager
...
- **Revocable at any time** — remove the IAM role, revoke the token at [myaccount.google.com/permissions](https://myaccount.google.com/permissions), or delete the connection in CompliBoss
...
<Accordion title="No projects listed after OAuth" icon="magnifying-glass">
  CompliBoss only lists projects the signed-in account has IAM access to. Sign in with a different Google account, or ask an admin to add your account as `roles/viewer` on the relevant projects.
</Accordion>
